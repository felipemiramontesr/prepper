  ...
  return (
    ...
    >
      <div className="mb-4 flex items-start justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-[.10em] text-[#fff]">{title}</h3>
        <Icon className="h-4 w-4 text-[var(--text-tertiary)] transition-colors group-hover:text-[var(--accent)]" />
      </div>

      <div
        className={`mb-1 text-3xl font-light tracking-tight ${valueClassName || 'text-[var(--text-primary)]'}`}
      >
        {value}
      </div>
